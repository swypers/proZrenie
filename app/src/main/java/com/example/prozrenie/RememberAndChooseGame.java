...
public class RememberAndChooseGame {
    ...
    public RememberAndChooseGame(String name, Map<Integer, Boolean> imageDict, LinearLayout imageLayout,
    ...
    void onFirstStart(){
        ...
        mKeyLayout.setVisibility(View.INVISIBLE);
    }
    void startGame(){
        new Thread(new Runnable() {
            public void run() {
                mMainImageView.post(new Runnable() {
                    public void run() {
                        mMainImageView.setVisibility(View.VISIBLE);
                    }
                });
                mKeyLayout.post(new Runnable() {
                    public void run() {
                        mKeyLayout.setVisibility(View.INVISIBLE);
                    }
                });
                ...
                mMainImageView.post(new Runnable() {
                    public void run() {
                        mMainImageView.setVisibility(View.INVISIBLE);
                    }
                });
                mKeyLayout.post(new Runnable() {
                    public void run() {
    ...
    void chooseLayoutWeight(ArrayList<Integer> iKeyImages){
        if (iKeyImages.size() > 4)
        {
            ...
            for (int i = 2; i < iKeyImages.size(); i++) {
